...
| GitHub Pages | 새 mission-B1-1 저장소 | 배포 완료 |
| README와 스크린샷 | README.md, images/*.png | 배포 화면 저장 완료 |
| 배포 URL에서 재검증 | API, 테마, 폼, 메뉴, 레이아웃 | 완료 |
...
브라우저 확인은 Codex 내장 브라우저에서 수행했습니다. 설치된 최신 Chrome에서의 별도 확인은 남아 있습니다. API 실패/빈 목록 등은 서버 상황을 강제로 바꾸지 않고 모의 응답으로 검증했습니다.

## 배포 후 확인

- 주소: https://tjdgns6427-ops.github.io/mission-B1-1/
- GitHub Pages의 배포 성공 및 공개 접속 확인.
- 실제 API에서 새 저장소를 포함한 11개 프로젝트 표시.
- 다크 모드 적용 및 새로고침 유지 확인.
- 빈 폼 오류 3개와 정상 입력 성공 안내 확인.
- 모바일 메뉴 열기, 프로젝트 링크 선택 후 닫기 확인.
- 390px / 768px / 1440px에서 문서 폭이 화면 폭을 넘지 않음.

## 제출 직전 남은 일

1. 본인의 Chrome에서 배포 URL을 열어 최종 확인.
2. 플랫폼에 저장소 URL과 배포 URL, 필요한 스크린샷 제출.
3. 제출 후 공개된 평가항목으로 추가 점검.
